...
export default function Weather_Widget() {
  ...
  const handleSearch = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://api.weatherapi.com/v1/current.json?key=63c2d9d22f4946b18c6161327240309&q=world&aqi=yes
=${process.env.NEXT_PUBLIC_WEATHER_API_KEY}&q=${trimmedLocation}`
      );
      if (!response.ok) {
